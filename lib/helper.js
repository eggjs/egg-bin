...
/**
 * Try to get framework path
 *
 * @param {String} cwd - current work path
 * @return {String} filepath
 */
exports.getFrameworkOrEggPath = function(cwd) {
  ...
  for (const name of names) {
    ...
    if (pkg.dependencies && pkg.dependencies.egg) {
      return path.join(moduleDir, name);
    }
  }

  // try to get egg
  const pkgfile = path.join(moduleDir, 'egg', 'package.json');
  if (fs.existsSync(pkgfile)) {
    return path.join(moduleDir, 'egg');
  }
